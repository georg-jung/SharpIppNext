...
{
    private static readonly Lazy<ExplicitTestsOptions> Options = new(() =>
    {
    ...
    private ISharpIppClient GetSharpIppClient =>
        new SharpIppClient(new HttpClient(new LoggingHandler(new HttpClientHandler())));

    private ISharpIppClient GetSharpIppClientWithoutLog => new SharpIppClient();
    ...
    public async Task PrintUriAsync_Full()
    {
        var request = new PrintUriRequest
        {
            ...
            {
                new(Tag.Keyword, "print-color-mode", "monochrome"),
            },
        };
        await TestRequestAsync(request, (client, r) => client.PrintUriAsync(r));
    }
    ...
    public async Task SendDocumentAsync_Multiple()
    {
        ...
        await TestJobRequestAsync(async jobId =>
        {
            ...
            await TestRequestAsync(request1,
                (client, r) =>
                {
                    r.JobId = jobId;
            ...
            return await TestRequestAsync(request2,
                (client, r) =>
                {
                    r.JobId = jobId;
                    return client.SendDocumentAsync(r);
    ...
    private async Task<TOut> TestRequestAsync<TIn, TOut>(
        TIn request,
        Func<ISharpIppClient, TIn, Task<TOut>> parseFunc) where TOut : IIppResponseMessage where TIn : IIppRequest
    {
        try
        {
    ...
    private async Task<TOut> TestJobRequestAsync<TIn, TOut>(
        TIn request,
        Func<ISharpIppClient, TIn, Task<TOut>> parseFunc) where TOut : IIppResponseMessage where TIn : IIppJobRequest
    {
        return await TestJobRequestAsync(jobId => TestRequestAsync(request,
            (client, r) =>
            {
                r.JobId = jobId;
